...
function fp_orphans_add_family_member($orphan,$name,$gender,$bd,$status){
	global $fp_handle;
	$n_orphan    = @mysql_real_escape_string(strip_tags($orphan),$fp_handle);
	$n_name    = @mysql_real_escape_string(strip_tags($name),$fp_handle);
	$n_gender = (int)$gender;
	$n_bd    = @mysql_real_escape_string(strip_tags($bd),$fp_handle);
	$n_status = (int)$status;
	
	
	$query = ("INSERT INTO `sibiling`(`id`,`name`,`sex`,`birth_date`,`state`) VALUE($n_orphan,'$n_name','$n_gender','$n_bd',$n_status)");
	$qresult = mysql_query($query);
