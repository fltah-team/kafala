...
//include("db.php");
		...
		// SELSECT ALL
	function fp_final_family_get($extra = ''){
		
		global $fp_handle ;
		$query = sprintf("SELECT * FROM `finalfamily` %s",$extra);
        ...
        function fp_final_family_get_num_rows(){
                global $fp_handle ;
                $query = sprintf("SELECT * FROM `finalfamily`");
                $qresult = @mysql_query($query);
	...
	function fp_final_family_get_by_id($id){
		$oid = (int)$id;
		if($oid == 0) return NULL ;
		
		$families = fp_final_family_get("WHERE `family_id` = ".$oid);
		if($families == NULL) return NULL ;
		$family = $families[0];
		return $family ;
		}
        function fp_final_family_get_last_id(){
                global $fp_handle ;
                $families = fp_final_family_get("ORDER BY `id` DESC LIMIT 1");
                if($families == NULL) return NULL ;
                $family = $families[0];
                return $family->id ;
        }
