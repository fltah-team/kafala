...
<?php 
	...
	$id = (int)$_GET['id']; 
        $check_orphan = fp_orphan_get_by_id($id);
        if($check_orphan)
        $file_name = "../orphan/updateOrphan.php";
        else $file_name = "../orphan/saveOrphan.php";
        echo $file_name;
...
function get_str(){
	var text = document.getElementsByTagName('input');
        ...
        window.location.href = <?php echo json_encode($file_name) ?>+"?"+str;
        //ajax(str);
        }
function ajax(str)
{		
	...
	elementID = "div";
	filename = <?php echo json_encode($file_name) ?>;
	post = false ;
    if (window.XMLHttpRequest)
    {
        ajax=new XMLHttpRequest();//IE7+, Firefox, Chrome, Opera, Safari
